...
def auto_query(model_provider,schema,user_question,extracted_classes,user_question_5w3h,filter): 
    ...
    user = f'''## USER QUESTION ##
        ...
        Below we have to query examples:
        ...
        SELECT DISTINCT ?uri ?texto ?tipo ?breadcrumb ?score
        FROM <https://omc.co/graph/5511993891773>
        WHERE {{
            {{
                ?uri rdf:type/rdfs:subClassOf* :Rule .
                ?uri (:appliesTo | :refersTo) ?classe .
                VALUES ?classe {{
                    :Processor
                    :Controller
                    :OnlyActOnDocumentedInstructionFromController
                    :ComplianceWithControllersInstructions
                }}
                ...
                ?q a luc-index:eu_full ;
                luc:query 'processor authority controller documented instruction personal data compliance circumstances' ;
                luc:entities ?uri .
                ?uri luc:score ?score .
        ...
        WHERE {{
            {{
                ...
                ?q a luc-index:eu_full ;
                luc:query 'Article 13 controller information provide data subject personal data obtained time transparency' ;
                luc:entities ?uri .
                ?uri luc:score ?score .
                ?uri rdf:type/rdfs:subClassOf* :Point .
                OPTIONAL {{ ?uri :fullText ?texto . }}
                OPTIONAL {{ ?uri :breadcrumb ?breadcrumb . }}
                FILTER( CONTAINS(LCASE(STR(?breadcrumb)), "article 13") ) 
                BIND(:Point AS ?tipo)
            }}
        }}
        ORDER BY DESC(?score) ?uri
        ...
        ## IMPORTANT ##   
        Lucene index MUST use always: {extracted_classes}
        All queries MUST use CONSTRUCT (not SELECT).
        The response must be valid TTL (text/turtle).   
