...
- Variáveis de ambiente:
  ...
  OMC_BASE_NS (ex.: https://omc.co/gdpr#)
...
GR_NS    = "https://omc.co/graph/"

INGEST_DIR              = Path(__file__).resolve().parent
ONTOLOGY_TTL_PATH       = INGEST_DIR / "_owl_gdpr.ttl"
PROMPT_ONTOLOGY_PATH    = ONTOLOGY_TTL_PATH
...
P_CHUNK_ID        = "chunkId"
P_CHUNK_TEXT      = "content"
P_CONTAINS        = "isContainedIn"      # Chunk -> Document
P_HAS_DOCUMENT    = "hasDocument"        # Entity -> Document
P_RESPONSAVEL_POR = "hasResponsible"     # Document -> Entity
P_EXTRACTED_FROM  = "isContainedIn"      # Entity -> Chunk
...
def _ontology_format(path: Path) -> str:
    return "turtle" if path.suffix.lower() in {".ttl", ".turtle"} else "xml"

def load_ontology_classes(path: Path = ONTOLOGY_TTL_PATH) -> set[str]:
    graph = Graph()
    graph.parse(str(path), format=_ontology_format(path))
    return {
        name
...
def load_ontology_object_properties(path: Path = ONTOLOGY_TTL_PATH) -> set[str]:
    graph = Graph()
    graph.parse(str(path), format=_ontology_format(path))
    return {
        name
...
def load_ontology_datatype_properties(path: Path = ONTOLOGY_TTL_PATH) -> set[str]:
    graph = Graph()
    graph.parse(str(path), format=_ontology_format(path))
    return {
        name
        for name in (_local_name(subject) for subject in graph.subjects(RDF.type, OWL.DatatypeProperty))
        if name
    }

PREFERRED_NODE_PROPS = {
    "name",
    "descricao",
    "articleReference",
    "number",
...
CANON_NODE_PROPS = load_ontology_datatype_properties()
#ALLOWED_NODE_PROPS = PREFERRED_NODE_PROPS & CANON_NODE_PROPS
ALLOWED_NODE_PROPS = {"descricao"}
...
def build_graph_transformer() -> LLMGraphTransformer:
    ...
    :appliesTo -> (:PersonalData or :SensitivePersonalData)
    :refersTo -> :Processing
    :hasLegalBasis -> the applicable legal basis when the text clearly states one
    :observesPrinciple -> the applicable GDPR principle when the text clearly states one

    4) Evidence:
    when an extracted :Rule or legal text element is tied to a chapter, article, point, or source chunk, use:
    :isContainedIn, :partOf, :hasChapter, :hasArticle, :hasPoint, :isPartOfChapter, :isPartOfArticle, or :isPartOfPoint
...
def rdf_namespace(base_ns: str) -> Namespace:
    base_ns = (base_ns or VOCAB_NS).strip()
    if base_ns.endswith(("#", "/")):
...
def graphdocs_to_rdflib(graph_doc, base_ns: str = VOCAB_NS) -> ConjunctiveGraph:
    ...
    V = rdf_namespace(base_ns)

    cg = ConjunctiveGraph()
    cg.bind("", V); cg.bind("gdpr", V)
    ...
    if doc_iri:
        ...
        if user_iri:
            ...
            g.add((doc_iri, V[P_RESPONSAVEL_POR], user_iri))
    ...
    for n in getattr(graph_doc, "nodes", []) or []:
        ...
        n_type = (getattr(n, "type", None) or "Entity").strip()
...
def upload_turtle(ttl: bytes | str, USUARIO: str) -> bool:    
    ...
    params  = {"context": f"<{GR_NS}{_safe(USUARIO)}>"}    
...
async def graph_ingest_gpdr_omc(data,debug=False):
