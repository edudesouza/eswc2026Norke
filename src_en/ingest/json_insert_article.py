...
# py -m src_en.ingest.json_insert_article
...
for index, item in enumerate(full_text,start=1):     
    ...
    if paragrafo == 'caput':     
        ...
        article         = item['artigo']  
        arr_article     = article.split(' ')
        article_number  = arr_article[1] 
        article_uri     = f'Article_{article_number}'
        article_title   = item['titulo_artigo']
        path            = f"{id}"

        rdf = f'''<https://omc.co/5511993891773/7492/{article_uri}>
        a v:{tipo} ; 
        v:articleNumber "{article_number}"  ;
        v:title "{article_title}"@en ;
