...
async def main(id,resposta_gt,user_id,pergunta,retrieval='grafo',retrieval_size=5,debug_all=False,debug_one=None,output=None,threshold=0.60):   
    ...
    if output:
        print('-> output ')
        csv_create(            
            output, id, 'grafo3', pergunta, resposta_gt, response_llm, 
...
async def run_batch(json_path='perguntas_gdpr_daily_qa_gpt_revisado.csv', output_csv='gdpr_daily_grafo3_gpt_0405_deepeval.csv'):
    ...
    for index, item in enumerate(perguntas[15:], start=15):     #rodar todas as perguntas a paorti da 1
