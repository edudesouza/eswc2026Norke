...
# Regex para texto plano do PDF GPDR (English version)
RE_CAPITULO  = re.compile(r"^\s*\*\*CHAPTER\s+([IVXLCDM]+)\s+(.+?)\*\*\s*$")
RE_ARTIGO    = re.compile(r"^\s*Article\s+(\d+)\s+(.*)$")
RE_PARAG     = re.compile(r"^\s*(\d+)\.\s+(.*)$")
# RE_PAR_UNICO não é comum na estrutura GDPR em inglês, mas mantemos a lógica de itens numerados via RE_PARAG

BASE_DIR   = os.path.dirname(os.path.abspath(__file__))
file_name  = os.path.join(BASE_DIR, 'pdf', 'GPDR_full.pdf')
full_text  = pymupdf4llm.to_markdown(file_name, header=False, footer=False)

print( full_text )

# Remove o exit() para permitir a execução do split_normativo
# Salva texto extraído em TXT para análise e auditoria
txt_path = "output/normativos_gpdr_v2.txt"
...
def split_normativo(md_text: str) -> List[Dict]:
    ...
    for line in lines:
        ...
        if m:
            flush()
            capitulo_nome = f"CHAPTER {m.group(1)} {m.group(2)}".strip()
            continue
        ...
        if m:
            flush()
            artigo_nr = f"Article {m.group(1)}".strip()
        ...
        # 3) Itens Numerados (Parágrafos no contexto GDPR)
        m = RE_PARAG.match(l)
        if m:
            flush()
            paragrafo_id = m.group(1).strip()
            rest = m.group(2).strip()
...
# Salva JSON
out_path = "output/chunks_normativos_gpdr_v2.json"
